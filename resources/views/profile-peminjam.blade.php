...
<head>
    ...
    <title>Profil Saya — Perpustakaan SDN Pasireurih</title>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .brand-logo { width: 40px; height: 40px; background: #fff; border-radius: 10px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; box-shadow: 0 4px 12px rgba(0,0,0,.2); }
        ...
        .nav-header-label { font-size: 9.5px; font-weight: 700; color: rgba(255,255,255,.3); letter-spacing: 2px; text-transform: uppercase; padding: 18px 16px 6px; }
        .nav-item {
        ...
        .nav-badge { margin-left: auto; background: #ef4444; color: #fff; font-size: 10px; font-weight: 700; padding: 2px 7px; border-radius: 99px; }
        ...
        .topbar-left { font-size: 15px; font-weight: 700; color: #0f1f5c; }
        .topbar-left i { margin-right: 6px; }
        .topbar-right { display: flex; align-items: center; gap: 8px; }

        .navbar-notif { position: relative; padding: 8px 10px; border-radius: 10px; color: #64748b; text-decoration: none; transition: all .2s; }
        .navbar-notif:hover { background: #f0f4ff; color: #0f1f5c; }
        .navbar-notif i { font-size: 17px; }
        .navbar-notif .badge-dot { position: absolute; top: 6px; right: 7px; width: 18px; height: 18px; background: #f59e0b; border-radius: 50%; font-size: 9px; font-weight: 700; color: #fff; display: flex; align-items: center; justify-content: center; border: 2px solid #fff; }

        .navbar-divider { width: 1px; height: 28px; background: #e8ecf4; margin: 0 8px; }

        .navbar-user-dropdown { position: relative; }
        .navbar-user-btn { display: flex; align-items: center; gap: 10px; padding: 6px 12px; border-radius: 10px; cursor: pointer; transition: background .2s; border: 1.5px solid #e8ecf4; background: transparent; outline: none; }
        ...
        .user-avatar-circle { width: 36px; height: 36px; border-radius: 50%; background: linear-gradient(135deg, #0f1f5c, #3b7eff); display: flex; align-items: center; justify-content: center; color: white; font-size: 14px; font-weight: 800; flex-shrink: 0; overflow: hidden; }
        ...
        .user-dropdown-menu { display: none; position: absolute; top: calc(100% + 12px); right: 0; width: 290px; background: #fff; border: 1px solid #e8ecf4; border-radius: 18px; box-shadow: 0 16px 56px rgba(8,15,46,.14); z-index: 9999; overflow: hidden; animation: dropFadeIn .18s ease; }
        @keyframes dropFadeIn { from { opacity:0; transform:translateY(-8px); } to { opacity:1; transform:translateY(0); } }
        .user-dropdown-menu.show { display: block; }
        .udm-hero { background: linear-gradient(155deg, #080f2e 0%, #0f1f5c 50%, #1a2e6f 100%); padding: 22px 20px 18px; display: flex; flex-direction: column; align-items: center; gap: 8px; position: relative; overflow: hidden; }
        ...
        .udm-action-btn { display: flex; align-items: center; gap: 10px; padding: 9px 10px; border-radius: 10px; font-size: 13px; font-weight: 600; color: #475569; text-decoration: none; transition: all .15s; cursor: pointer; border: none; background: transparent; width: 100%; text-align: left; }
        ...
        .ph-title { font-size: 22px; font-weight: 800; color: #0f172a; margin: 0; letter-spacing: -.02em; }
        .ph-sub   { font-size: 13px; color: #94a3b8; margin-top: 3px; font-weight: 400; }

        .breadcrumb-chip { display: flex; align-items: center; gap: 6px; background: #fff; border: 1px solid #e8ecf4; border-radius: 10px; padding: 8px 16px; font-size: 13px; font-weight: 600; color: #64748b; box-shadow: 0 1px 4px rgba(0,0,0,.05); }
        .breadcrumb-chip i { color: #0f1f5c; }
        ...
        /* ---- Left card ---- */
        /* PENTING: tidak pakai overflow:hidden agar avatar yang margin-top negatif tidak terpotong */
        .profile-card { background: #fff; border-radius: 18px; border: 1px solid #e8ecf4; box-shadow: 0 2px 10px rgba(8,15,46,.05); }

        .profile-banner {
            height: 90px;
            background: linear-gradient(155deg, #080f2e 0%, #0f1f5c 50%, #1a2e6f 100%);
            position: relative;
            /* border-radius hanya sudut atas, bukan overflow:hidden agar avatar tidak terpotong */
            border-radius: 18px 18px 0 0;
            overflow: hidden; /* overflow hanya di banner, bukan di card keseluruhan */
        }
        ...
        .profile-banner::after {
            ...
            background: rgba(245,158,11,.12); border-radius: 50%;
        }

        .profile-body { padding: 0 24px 28px; }

        /* Avatar ditaruh di luar overflow, posisi absolute relatif ke .profile-card-wrap */
        .profile-avatar-wrap { margin-top: -38px; margin-bottom: 16px; position: relative; z-index: 2; }

        .profile-avatar {
            width: 76px; height: 76px; border-radius: 50%;
            background: linear-gradient(135deg, #f59e0b, #fbbf24);
            border: 4px solid #fff; box-shadow: 0 6px 20px rgba(8,15,46,.15);
        ...
        .profile-avatar-placeholder {
            ...
            border: 4px solid #fff; box-shadow: 0 6px 20px rgba(8,15,46,.15);
            display: flex; align-items: center; justify-content: center;
            font-size: 28px; font-weight: 800; color: #0f1f5c;
        }

        .profile-name { font-size: 16px; font-weight: 800; color: #0f172a; margin-bottom: 4px; letter-spacing: -.02em; }
        ...
        .profile-role-badge {
            ...
            border: 1px solid #c7d7fd; margin-bottom: 18px;
        }
        .profile-role-badge i { font-size: 10px; color: #f59e0b; }
        ...
        .info-label { font-size: 9.5px; font-weight: 700; color: #94a3b8; text-transform: uppercase; letter-spacing: .5px; display: block; }
        .info-value { font-size: 13px; font-weight: 600; color: #1e293b; display: block; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }

        /* ---- Right edit card ---- */
        .edit-card { background: #fff; border-radius: 18px; border: 1px solid #e8ecf4; box-shadow: 0 2px 10px rgba(8,15,46,.05); overflow: hidden; }
        ...
        .edit-header::before { content: ''; position: absolute; inset: 0; background-image: radial-gradient(circle, rgba(255,255,255,.06) 1px, transparent 1px); background-size: 20px 20px; pointer-events: none; }
        .edit-header-icon { width: 40px; height: 40px; background: rgba(245,158,11,.2); border: 1px solid rgba(245,158,11,.4); border-radius: 10px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; position: relative; z-index: 1; }
        .edit-header-icon i { font-size: 16px; color: #fbbf24; }
        .edit-header-texts { position: relative; z-index: 1; }
        .edit-header-texts h4 { font-size: 15px; font-weight: 800; color: #fff; margin: 0; letter-spacing: -.01em; }
        .edit-header-texts p { font-size: 11.5px; color: rgba(255,255,255,.55); margin: 2px 0 0; font-weight: 400; }

        .edit-body { padding: 28px 28px 32px; }

        /* Avatar preview */
        .preview-wrap { display: flex; align-items: center; gap: 20px; margin-bottom: 28px; padding: 20px; background: #f8faff; border-radius: 14px; border: 1px solid #e8ecf4; }
        ...
        .preview-avatar {
            ...
            box-shadow: 0 4px 14px rgba(8,15,46,.1);
        }
        ...
        .preview-avatar-placeholder {
            ...
            box-shadow: 0 4px 14px rgba(8,15,46,.1);
            display: flex; align-items: center; justify-content: center;
            font-size: 30px; font-weight: 800; color: #0f1f5c;
        }

        .preview-info { flex: 1; }
        .preview-info h5 { font-size: 13.5px; font-weight: 700; color: #0f172a; margin: 0 0 4px; }
        .preview-info p  { font-size: 12px; color: #94a3b8; margin: 0 0 12px; font-weight: 400; }
        ...
        .file-input-label:hover { background: #e0e8ff; border-color: #3b7eff; }
        .file-input-label i { font-size: 12px; }
        input[type="file"] { display: none; }

        /* Form fields */
        .form-section { margin-bottom: 20px; }

        .form-label { display: block; font-size: 11.5px; font-weight: 700; color: #64748b; text-transform: uppercase; letter-spacing: .5px; margin-bottom: 8px; }

        .form-control {
            width: 100%; padding: 11px 14px;
            border: 1.5px solid #e8ecf4; border-radius: 10px;
            font-family: 'Sora', sans-serif; font-size: 13.5px; font-weight: 600; color: #0f172a;
            background: #fff; transition: all .2s; outline: none;
        }
        .form-control:focus { border-color: #0f1f5c; box-shadow: 0 0 0 3px rgba(15,31,92,.08); }
        .form-control:disabled { background: #f8faff; color: #94a3b8; cursor: not-allowed; }
        ...
        .save-btn:hover { background: linear-gradient(135deg, #fbbf24, #f59e0b); box-shadow: 0 6px 22px rgba(245,158,11,.4); transform: translateY(-1px); }
        ...
        .pl-text {
            ...
            animation: dotDot 1.4s steps(3, end) infinite;
        }
        @keyframes dotDot {
            0%   { content: 'Memuat halaman'; }
            33%  { opacity: .6; }
            66%  { opacity: .4; }
            100% { opacity: 1; }
        }

        /* Halaman masuk setelah loader */
        .main-wrapper, .sidebar { opacity: 0; }
        .main-wrapper.loaded, .sidebar.loaded {
...
<div class="main-wrapper">
    ...
    <div class="page-content">
        ...
        <div class="page-header">
            <div>
                <h4 class="ph-title">Profile Saya</h4>
                <p class="ph-sub">Kelola informasi akun dan foto profil Anda.</p>
            </div>
            <div class="breadcrumb-chip">
                <i class="fas fa-home"></i> Dashboard / <strong style="color:#0f1f5c;">Profile</strong>
            </div>
        </div>
        ...
        <div class="profile-grid">
            ...
            <div class="profile-card">
                ...
                <div class="profile-body">
                    <div class="profile-avatar-wrap">
                        @if(auth()->user()->foto)
                            <img src="{{ asset('storage/' . auth()->user()->foto) }}"
                                 class="profile-avatar" id="mainPreview">
                        @else
                            <div class="profile-avatar-placeholder" id="mainPreviewText">
                                {{ strtoupper(substr(auth()->user()->name, 0, 1)) }}
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="profile-role-badge">
                        ...
                        {{ ucfirst(auth()->user()->getRoleNames()->first() ?? 'Peminjam') }}
                    </div>

                    <div class="info-list">
                        <div class="info-item">
                            <div class="info-icon icon-bg-purple"><i class="fas fa-envelope"></i></div>
                            <div class="info-text">
                                <span class="info-label">Email</span>
                                <span class="info-value">{{ auth()->user()->email }}</span>
                            </div>
                        ...
                        <div class="info-item">
                            <div class="info-icon icon-bg-green"><i class="fas fa-shield-alt"></i></div>
                            <div class="info-text">
                                <span class="info-label">Role</span>
                                <span class="info-value">{{ ucfirst(auth()->user()->getRoleNames()->first() ?? 'Peminjam') }}</span>
                            </div>
                        </div>
                        <div class="info-item">
                            <div class="info-icon icon-bg-gold"><i class="fas fa-calendar-alt"></i></div>
                            <div class="info-text">
                                <span class="info-label">Bergabung</span>
            ...
            <div class="edit-card">
                <div class="edit-header">
                    ...
                    <div class="edit-header-texts">
                        <h4>Edit Profil</h4>
                        <p>Perbarui data dan foto profil Anda</p>
                    </div>
                </div>
                    ...
                    <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="preview-wrap">
                            <div class="preview-avatar-wrap">
                                @if(auth()->user()->foto)
                                    <img src="{{ asset('storage/' . auth()->user()->foto) }}"
                                         class="preview-avatar" id="previewImage">
                                @else
                                    <div class="preview-avatar-placeholder" id="previewText">
                                        {{ strtoupper(substr(auth()->user()->name, 0, 1)) }}
                                    </div>
                                    <img id="previewImage" class="preview-avatar" style="display:none;">
                                @endif
                            </div>
...
document.getElementById('fotoInput').addEventListener('change', function(e) {
    ...
    if (file) {
        ...
        reader.onload = function(ev) {
            ...
            let txt = document.getElementById('previewText');
...
window.addEventListener('load', function () {
    // Minimum tampil 1.7 detik supaya animasi progress terlihat
    setTimeout(function () {
